...
def render_attendance():
    ...
    with col2.container(border=True):
        ...
        if st.session_state.att_meetings:
            selected_meeting_to_delete = st.selectbox(
                "Select meeting to delete",
                st.session_state.att_meetings,
                format_func=lambda x: x["name"],
                key="att_select_delete_meeting"
            )
            
            if st.button("Delete Meeting", key="att_btn_delete_meeting", type="secondary"):
                if attendance_sheet and sheet_handler:
                    try:
                        # 从Google Sheets删除相关记录
                        all_rows = attendance_sheet.get_all_values()
                        # 从后往前删除，避免行号变动影响
                        for i in range(len(all_rows)-1, 0, -1):
                            if all_rows[i][2] == str(selected_meeting_to_delete["id"]):
                                attendance_sheet.delete_rows(i+1)  # 表格行号从1开始
                        
                        # 从本地状态删除
                        st.session_state.att_meetings = [
                            m for m in st.session_state.att_meetings 
                            if m["id"] != selected_meeting_to_delete["id"]
                        ]
                        # 删除相关考勤记录
                        st.session_state.att_records = {
                            (m_id, mt_id): val for (m_id, mt_id), val 
                            in st.session_state.att_records.items() 
                            if mt_id != selected_meeting_to_delete["id"]
                        }
                        
                        st.success(f"Deleted meeting: {selected_meeting_to_delete['name']}")
                        sync_data()  # 同步数据刷新表格
                    except Exception as e:
                        st.warning(f"删除会议失败: {str(e)}")
    ...
    with col3.container(border=True):
            ...
            if st.button("Set All Present", key="att_btn_set_all_present"):
                if attendance_sheet and sheet_handler and st.session_state.att_members:
                    try:
                        ...
                        st.warning(f"Failed to update all attendance: {str(e)}")
        ...
        if st.session_state.att_members and st.session_state.att_meetings:
            ...
            is_present = st.checkbox(
                ...
                key="att_check_present"
            )
            
            if st.button("Save Attendance", key="att_btn_save"):
                # 更新本地状态
